...
public class HatcherZombie : MonoBehaviour, IDamageable
{
    [SerializeField] public int maxHealth = 1;
    [SerializeField] private SpriteRenderer healthBar;
    [SerializeField] private SpriteRenderer healthFill;
    [SerializeField] private int GoldValue = 10;
    [SerializeField] private GameObject spawnOnDeathPrefab; // Specify the zombie prefab to spawn on death
    [SerializeField] private int spawnOnDeathAmount = 1; // Adjustable variable for the amount of spawns on death
    ...
    void HandleDeath()
    {
        ...
        // Spawn additional zombies on death if specified
        for (int i = 0; i < spawnOnDeathAmount; i++)
        {
            if (spawnOnDeathPrefab != null)
            {
                GameObject spawnedZombie = Instantiate(spawnOnDeathPrefab, transform.position, Quaternion.identity);

                // Pass waypoints to the spawned zombie using the new method
                NormalZombie spawnedEnemyWaypoint = spawnedZombie.GetComponent<NormalZombie>();
                if (spawnedEnemyWaypoint != null)
                {
                    spawnedEnemyWaypoint.waypoints = (Transform[])waypoints;
                    spawnedEnemyWaypoint.currentWaypoint = currentWaypoint;
                }
            }
        }
